...
import io.legado.app.core.platform.Clock
import io.legado.app.core.platform.DefaultDispatcherSet
import io.legado.app.core.platform.DispatcherSet
import io.legado.app.core.platform.SystemClock
...
class WholeBookPageCoordinator(
    ...
    /**
     * IO 密集型调度器：章节长度加载与精确页数读写都走它。
     *
     * 平台侧应传 IO 线程池（Android 为 `Dispatchers.IO`）。`Dispatchers.IO` 不在 `commonMain`
     * 里，所以这里要求显式注入而不给默认值 —— 默认到 `Dispatchers.Default` 会让生产路径
     * 静默退化成 CPU 线程池，而这是行为变化不是能力缺失。
     */
    private val ioDispatcher: CoroutineDispatcher,
    /** 仅用于测试，生产路径使用 [HeuristicPageEstimator]。 */
    private val estimator: ChapterPageEstimator? = null,
    private val clock: Clock = SystemClock,
    private val dispatcherSet: DispatcherSet = DefaultDispatcherSet,
    ...
    fun requestEstimate(
        ...
        val requestStartedAt = clock.nowNanos()
        ...
        val job = scope.launch(dispatcherSet.default) {
            val chapters = withContext(ioDispatcher) { chapterLoader() }
            ...
            val cachedExactCounts = withContext(ioDispatcher) {
                runCatching {
            ...
            if (changed) {
                metrics.record(
                    PageEstimateMetric.EstimateCompleted(
                        ...
                        durationMillis = (clock.nowNanos() - requestStartedAt) / 1_000_000L,
                    )
                )
                onStateChanged()
            }
