...
sealed interface ReplaceRuleEffect {
     ...
     * 弹一条轻提示（snackbar）。
     *
     * M1-3x 新增：迁移前导入落库后的「成功导入 N 条规则」由 `BaseRuleViewModel` 的
     * `_eventChannel` 发出，现在落库归 `RuleTransferUseCase`，它不产出业务文案 ⇒ 由 VM 转成
     * effect，界面自己弹。文案仍是硬编码中文（与迁移前逐字相同，原本也未走资源）。
     */
    data class ShowMessage(val message: String) : ReplaceRuleEffect
}
