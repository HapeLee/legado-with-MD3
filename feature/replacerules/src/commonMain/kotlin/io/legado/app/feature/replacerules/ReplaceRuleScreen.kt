...
import io.legado.app.feature.replacerules.res.a11y_more_actions_for
import io.legado.app.feature.replacerules.res.a11y_rule_enabled_switch
import io.legado.app.feature.replacerules.res.add
import io.legado.app.feature.replacerules.res.cancel
import io.legado.app.feature.replacerules.res.delete
import io.legado.app.feature.replacerules.res.disable_selection
import io.legado.app.feature.replacerules.res.disabled
import io.legado.app.feature.replacerules.res.drag_disabled_in_sort_mode
import io.legado.app.feature.replacerules.res.enable
import io.legado.app.feature.replacerules.res.enabled
import io.legado.app.feature.replacerules.res.export
import io.legado.app.feature.replacerules.res.group_management
import io.legado.app.feature.replacerules.res.import_on_line
import io.legado.app.feature.replacerules.res.import_replace_rule
import io.legado.app.feature.replacerules.res.import_str
import io.legado.app.feature.replacerules.res.move_to_bottom
import io.legado.app.feature.replacerules.res.move_to_top
import io.legado.app.feature.replacerules.res.ok
import io.legado.app.feature.replacerules.res.replace_purify
import io.legado.app.feature.replacerules.res.replace_purify_search
import io.legado.app.feature.replacerules.res.sort_name_asc
import io.legado.app.feature.replacerules.res.sort_name_desc
import io.legado.app.feature.replacerules.res.sort_new_first
import io.legado.app.feature.replacerules.res.sort_old_first
import io.legado.app.feature.replacerules.res.to_bottom
import io.legado.app.feature.replacerules.res.to_top
...
fun ReplaceRuleScreen(
    ...
    effects: Flow<ReplaceRuleEffect>,
    groups: List<String>,
    ...
    // 迁移前这里是 `context.getString(Res.string.drag_disabled_in_sort_mode)`——`Context` 只能
    // 留在 Route 侧，故改成在 Composition 里取 CMP 资源（同一条文案，同一个资源名）。
    val dragDisabledText = stringResource(Res.string.drag_disabled_in_sort_mode)
    ...
    FilePickerSheet(
        ...
        onSelectSysDir = {
            ...
            onPickExportTarget("exportReplaceRule.json")
        },
        onUpload = {
            showExportSheet = false
    ...
    LaunchedEffect(Unit) {
        effects.collect { effect ->
            when (effect) {
                // 导入落库后的「成功导入 N 条规则」：迁移前走 `_eventChannel`，现由 VM 发 effect。
                is ReplaceRuleEffect.ShowMessage -> snackbarHostState.showSnackbar(effect.message)
                // `OpenReplaceEditor` 迁移前后都没有界面消费（`emitOpenReplaceEditor` 无调用方），
                // 本分支只为穷尽 sealed 接口。
                is ReplaceRuleEffect.OpenReplaceEditor -> Unit
            }
        }
    }
    ...
    RuleListScaffold(
        ...
        searchPlaceholder = stringResource(Res.string.replace_purify_search),
        ...
        selectionSecondaryActions = listOf(
            ...
            ActionItem(
                text = stringResource(Res.string.disable_selection),
                onClick = { onIntent(ReplaceRuleIntent.DisableSelection) }
            ),
            ActionItem(
                text = stringResource(Res.string.to_top),
                onClick = { onIntent(ReplaceRuleIntent.TopSelectByIds(selectedIds)) }
            ),
            ActionItem(
                text = stringResource(Res.string.to_bottom),
        ...
        floatingActionButton = {
            AppFloatingActionButton(
                ...
                tooltipText = stringResource(Res.string.add),
        ...
        dropDownMenuContent = { dismiss ->
            ...
            RoundDropdownMenuItem(
                text = stringResource(Res.string.group_management),
                onClick = { showGroupManageSheet = true; dismiss() }
            )
            PillDivider()
            RoundDropdownMenuItem(
                text = stringResource(Res.string.sort_old_first),
                onClick = { onIntent(ReplaceRuleIntent.SetSortMode("asc")); dismiss() }
            )
            RoundDropdownMenuItem(
                text = stringResource(Res.string.sort_new_first),
                onClick = { onIntent(ReplaceRuleIntent.SetSortMode("desc")); dismiss() }
            )
            RoundDropdownMenuItem(
                text = stringResource(Res.string.sort_name_asc),
                onClick = {
                    onIntent(ReplaceRuleIntent.SetSortMode("name_asc"))
                    dismiss()
                    scope.launch {
                        snackbarHostState.showSnackbar(dragDisabledText)
                    }
                }
            )
            RoundDropdownMenuItem(
                text = stringResource(Res.string.sort_name_desc),
                onClick = {
            ...
            FastScrollLazyColumn(
                ...
                items(rules, key = { it.id }) { ui ->
                    val enabledState = stringResource(
                        if (ui.isEnabled) Res.string.enabled else Res.string.disabled
                    )
                    ...
                    ReorderableSelectionItem(
                        ...
                        editContentDescription = stringResource(Res.string.a11y_edit_named, ui.name),
                        moreContentDescription = stringResource(
                            Res.string.a11y_more_actions_for,
                            ui.name
                        ),
                        onClickEdit = {
                            onNavigateToEdit(
