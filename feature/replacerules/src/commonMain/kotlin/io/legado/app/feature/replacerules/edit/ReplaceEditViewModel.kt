...
class ReplaceEditViewModel(
    ...
    private fun copyRule() {
        viewModelScope.launch(Dispatchers.Main) {
            ...
            val json = JsonCodec.toJson(ruleToCopy)
            // `Clipboard.setText` 与迁移前的 `ClipboardProvider.current.setText` 是**同一个
            // 契约实例**（Provider 只是它的注入点）⇒ 写入 + 一次「复制完成」提示的语义不变。
    ...
    private fun pasteRule() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // `ReplaceRule` 在 app 侧 `GSON` 里没有注册自定义 deserializer ⇒
                // `JsonCodec`（配置与 `INITIAL_GSON` 严格对齐）结果一致。
                val pastedRule = JsonCodec.fromJsonObject(text, ReplaceRule::class)
    ...
    private fun save() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val rule = (existingRule ?: ReplaceRule()).apply {
                id = existingRule?.id ?: if (state.id <= 0) systemTimeMillis() else state.id
                name = state.name
                group = if (state.group == "默认" || state.group.isBlank()) null else state.group
                pattern = state.pattern
                replacement = state.replacement
