...
| `ui/book/toc`、`ui/book/toc/rule`、`ui/book/toc/rule/preview` | `feature/table-of-contents` + `feature/txttocrules` | Feature + platform adapter | **拆三域（2026-09-09 审计）**：① `rule` 子域 = TXT 目录规则管理，**`:feature:txttocrules` 已建**：`TxtRuleScreen`（含 `TxtRuleRouteScreen`）/`TxtTocRuleContract`/`TxtTocRuleViewModel` 迁入 `io.legado.app.feature.txttocrules`（3 文件 + 26 R.string×4 语言）；`TxtTocRuleActivity` 因依赖 `BaseComposeActivity` **留 `:app` 作薄宿主**（只 import 模块 `TxtRuleRouteScreen`），`MyScreen` 入口与 Manifest 不变；VM 的「导入内置规则」原直连 `DefaultData`（appDb+assets，不可下沉）→ 抽 `:core:viewmodel` 契约 `BuiltInRulesImporter`（impl `AndroidBuiltInRulesImporter` 留 app，Koin 注入）。② `rule/preview` 子域依赖 app 的 `model.localBook.LocalBook`（本地书解析）→ **platform island，不迁**。③ `toc` 主域（TocActivity/Screen/ViewModel，书籍目录页）属阅读主链，**留 `:app`** |
