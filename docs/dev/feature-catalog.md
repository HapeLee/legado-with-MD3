...
| `feature/replacerules/{,edit}`（原 `ui/replace`） | `feature/replace-rules` | Feature domain | 已迁 Stage A 包（2026-09-08）：`io.legado.app.feature.replacerules[.edit]`，Manifest 与测试路径已同步。**遗留跨 Feature 引用**：`ReplaceEditRoute`/`ReplaceRuleActivity` 被 reader、toc、my 三个包直接 import，Stage B 前须收口成导航契约 |
| `ui/dict`                                | `feature/dictionary`        | Feature                       | rule editor 留同域，执行引擎经 domain/platform contract                               |
| `feature/tagrules/{highlight,group}`（原 `ui/highlightTagRule`、`ui/tagGroupRule`） | `feature/tag-rules` | Feature domain | 已迁 Stage A 包：`io.legado.app.feature.tagrules.{highlight,group}`（2026-09-08）；Contract 已去平台化（Uri→String），共享 UI 状态叶子已下沉 designsystem；第二批已收直接依赖（ViewModel 注入 `HighlightTagRuleRepository`/`TagGroupRuleRepository` 取代 `XxxRepository(appDb)`），并删除零引用类型 `TagGroupRuleEffect`、两个 `*RenderState`；仍留 `android.net.Uri.parse` 与 `context.toastOnUi` 在 ViewModel 层，待 `BaseRuleViewModel` 6 个子类统一收口 |
