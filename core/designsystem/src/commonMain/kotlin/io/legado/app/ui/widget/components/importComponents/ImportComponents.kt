...
import io.legado.app.core.designsystem.res.back
import io.legado.app.core.designsystem.res.cancel
import io.legado.app.core.designsystem.res.deselect_all
import io.legado.app.core.designsystem.res.details
import io.legado.app.core.designsystem.res.edit_not_supported
import io.legado.app.core.designsystem.res.error
import io.legado.app.core.designsystem.res.history_label
import io.legado.app.core.designsystem.res.import_action
import io.legado.app.core.designsystem.res.import_status_error
import io.legado.app.core.designsystem.res.import_status_existing
import io.legado.app.core.designsystem.res.import_status_new
import io.legado.app.core.designsystem.res.import_status_update
import io.legado.app.core.designsystem.res.loading
import io.legado.app.core.designsystem.res.ok
import io.legado.app.core.designsystem.res.select_all
import io.legado.app.core.designsystem.res.select_count
 ...
 * 当前窗口可用高度（dp）。
 *
 * 迁移前这里读的是 `LocalConfiguration.current.screenHeightDp`——`LocalConfiguration` 是
 * Android-only（`androidx.compose.ui.platform` 里 desktop 没有它）；本文件用它给多层 sheet
 * 限高。CMP 侧拿等价信息要走 `LocalWindowInfo.containerSize`（同 M1-3p 在
 * `AppContainerBackground` 里的处理），它给的是**像素**，所以用 `LocalDensity` 转成 dp。
 *
 * ⚠️ 窗口尚未完成布局时 `containerSize` 可能是 0，`heightIn(max = 0.dp)` 会让那一帧把内容
 * 压成零高；窗口信息是可观察状态，布局后会立刻重组成正确高度——这与 Android 侧
 * `screenHeightDp`（来自 Configuration，初值即正确）的差异仅存在于首帧。
 */
@Composable
private fun maxWindowHeightDp(): Dp = with(LocalDensity.current) {
        ...
        content = {
            Column {
                ...
                if (historyValues.isNotEmpty()) {
                    ...
                    AppText(stringResource(Res.string.history_label), style = LegadoTheme.typography.labelSmall)
...
fun <T> BatchImportDialog(
    ...
    // M2-1：原先这里走 `ImportJsonEditorProvider.current`（全局单例）。
    // designsystem 是通用组件库，不该认识 service locator；由调用方（Route / Screen）
    // 把 host composition root 绑定的实现传进来。
    ...
    AppModalBottomSheet(
        show = show,
        onDismissRequest = onDismissRequest,
        modifier = Modifier.heightIn(max = maxWindowHeightDp() * 0.8f),
        title = sheetTitle,
        ...
        } else {
            {
                Row(
                    ...
                    MediumTonalButton(
                        ...
                        contentDescription = stringResource(if (allSelected) Res.string.deselect_all else Res.string.select_all)
                    )
                }
            }
        },
        ...
        if (isEditing) {
            ...
            Box(
                modifier = Modifier
                    .fillMaxWidth()
                    .heightIn(max = maxWindowHeightDp() * 0.58f)
            ) {
                LazyColumn(
...
private fun <T> BatchImportJsonEditContent(
    ...
    if (fields == null) {
        AppText(stringResource(Res.string.edit_not_supported))
        return
    }
...
fun ImportItemRow(
    ...
    SelectionItemCard(
        ...
        trailingAction = {
            AppText(
                text = when (status) {
                    ImportStatus.New -> stringResource(Res.string.import_status_new)
                    ImportStatus.Update -> stringResource(Res.string.import_status_update)
                    ImportStatus.Existing -> stringResource(Res.string.import_status_existing)
                    ImportStatus.Error -> stringResource(Res.string.import_status_error)
                },
