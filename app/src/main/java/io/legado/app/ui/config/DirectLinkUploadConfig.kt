...
class DirectLinkUploadConfig : BaseBottomSheetDialogFragment(R.layout.dialog_direct_link_upload_config),
    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        when (item?.itemId) {
            ...
            R.id.menu_paste_rule -> runCatching {
                ...
                toastOnUi("Clipboard is empty or invalid format")
            }
        }
        return true
    }
    ...
    private fun getRule(): DirectLinkUpload.Rule? {
        ...
        if (uploadUrl.isNullOrBlank()) {
            toastOnUi("Upload URL cannot be empty")
            return null
        }
        if (downloadUrlRule.isNullOrBlank()) {
            toastOnUi("Download URL rule cannot be empty")
            return null
        }
        if (summary.isNullOrBlank()) {
            toastOnUi("Comment cannot be empty")
            return null
        }
