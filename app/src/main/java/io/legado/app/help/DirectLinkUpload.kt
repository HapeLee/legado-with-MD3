...
object DirectLinkUpload {
    ...
    suspend fun upLoad(
        ...
        if (url.isBlank()) {
            throw NoStackTraceException("Upload URL not configured")
        }
        val downloadUrlRule = rule.downloadUrlRule
        if (downloadUrlRule.isBlank()) {
            throw NoStackTraceException("Download URL rule not configured")
        }
        ...
        if (downloadUrl.isBlank()) {
            throw NoStackTraceException("Upload failed,${res.body}")
        }
        return downloadUrl
    }
